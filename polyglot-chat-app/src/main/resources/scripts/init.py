...
def transform_message(sender, msg):
    if SIA is not None:
        compound = SIA.polarity_scores(msg)['compound']
        if compound > 0.3:
            emoji = "😀"
        elif compound > -0.3:
            emoji = "🫥"
        ...
        emoji = "💬"
    return f"[{sender} {emoji}] {msg}"


def validate(senderTopic, receiverTopic):
    print(f"Called from python with {senderTopic=}, {receiverTopic=}")
